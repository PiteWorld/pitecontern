...
import AppCommunicate.StatusBase.ComStatue;
import android.util.Log;
import android_serialport_api.SerialPortService;

public class ThreadReader extends Thread implements ModelWorkDT, ModelWork, ModelWorkID {

	private Socket so = null;
	private OutputStream ou = null;
	private InputStream in = null;
	private int id = 0;
	private StatusBase status = null; // 起始状态
	...
	public ThreadReader(Socket so, ReaderWork work) throws IOException {
		this.so = so;
	...
	public void setTimer() {
		...
		timer = new Timer();
		timer.schedule(task, TimerUtil.time, TimerUtil.time);
	}
	...
	private void updateStatus(byte[] bt) {
		...
		status.setID(id);
		try {
			if (status.getCurStatus() != ComStatue.CHECK)
				status.StatusRun(ou);
		} catch (Exception e) {
			e.printStackTrace();
		}
